...
def oncelik_kesmeli_calistir(surecler):
    kalanlar = []
    for s in surecler:
        kalanlar.append({
            "id": s["id"],
    ...
    while bitti < toplam:
        ...
        if not hazir:
            sonraki = min([x["g"] for x in kalanlar if x["k"] > 0])
...
def oncelik_kesmeli_rapor(surecler, dosya_adi):
    tablo = oncelik_kesmeli_calistir(surecler)
    ...
    with open(dosya_adi, "w", encoding="utf-8") as f:
        f.write("ONCELIK KESMELI ZAMAN TABLOSU\n")
        for b, p, s in tablo:
            f.write(f"[ {b} ] -- {p} -- [ {s} ]\n")
